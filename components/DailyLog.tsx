  ...
  return (
    <div className="flex flex-col xl:flex-row gap-4 md:gap-8 h-full">
      ...
      <div className="flex-1 flex flex-col gap-4 md:gap-6 overflow-hidden">
          <div className="flex items-center justify-between bg-white p-3 md:p-4 rounded-2xl shadow-sm border border-slate-200">
             <div className="flex items-center gap-2 md:gap-4">
                 <div className="bg-indigo-100 p-2 rounded-lg text-indigo-600">
                     <Calendar size={20} />
                 </div>
                 <div>
                     <h2 className="text-lg md:text-xl font-bold text-slate-800">{date}</h2>
                     <p className="text-xs text-slate-500">Daily Log & Protocol</p>
                 </div>
             </div>
             <div className="flex items-center gap-2 md:gap-3">
                {isSaving && <span className="text-xs text-indigo-500 font-medium animate-pulse">Saving...</span>}
                <button
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar space-y-6 pb-10">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="p-6">
                    <form onSubmit={handleAddTodo} className="mb-6 space-y-3">
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-1 px-4 py-2 bg-white text-slate-900 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none"
                            />
                            <button type="submit" className="bg-indigo-600 text-white p-2 rounded-lg hover:bg-indigo-700 transition shadow-md shadow-indigo-200">
                                <Plus size={20} />
                            </button>
                        </div>
                        <select 
                            ...
                            className="w-full text-xs bg-white text-slate-700 border border-slate-300 rounded-lg px-3 py-2 outline-none cursor-pointer"
                        >
                    ...
                    <div className="space-y-2">
                        {todos.length === 0 && <p className="text-center text-slate-400 text-sm py-4 italic">No tasks set for today.</p>}
                        {todos.map(todo => (
                            <div key={todo.id} className="group flex items-center gap-3 p-3 rounded-lg border border-slate-100 hover:border-indigo-100 bg-white transition shadow-sm hover:shadow-md">
                                <button onClick={() => toggleTodo(todo.id)} className="text-slate-300 hover:text-green-500 transition">
                                    {todo.completed ? <CheckCircle className="text-green-500 w-5 h-5" /> : <Circle className="w-5 h-5" />}
                                </button>
                                ...
                                <button onClick={() => deleteTodo(todo.id)} className="opacity-0 group-hover:opacity-100 text-slate-300 hover:text-red-400 transition">
                                    <Trash2 size={16} />
                                </button>
                            </div>
                        ))}
            ...
            {/* Journal Section */}
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col h-[300px] md:h-[350px]">
                ...
                <div className="flex-1 p-2 overflow-hidden">
                    <textarea
                        ...
                        className="w-full h-full p-4 bg-white text-slate-800 resize-none outline-none text-base leading-relaxed overflow-y-auto"
                    ></textarea>
                </div>
            </div>
          </div>
      ...
      <div className="w-full xl:w-72 flex-shrink-0">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden h-full max-h-[calc(100vh-120px)] flex flex-col">
              <div className="p-4 border-b border-slate-100 bg-slate-50 flex items-center gap-2 sticky top-0">
                  <History size={16} className="text-slate-500" />
                  <h3 className="font-bold text-slate-700 text-sm uppercase tracking-wide">Recorded Archives</h3>
              </div>
                  ...
                  {sortedHistoryDates.map(d => (
                      <div 
                        ...
                        className={`w-full rounded-lg text-sm font-medium transition flex justify-between items-center group pr-2 select-none cursor-pointer ${
                            d === date ? 'bg-indigo-50 border border-indigo-100 shadow-sm' : 'hover:bg-slate-50 border border-transparent'
                        }`}
                        onClick={() => onChangeDate(d)}
                      >
                        <div className={`flex-1 px-4 py-3 flex items-center gap-2 ${d === date ? 'text-indigo-700' : 'text-slate-600'}`}>
                              <Clock size={12} className={d === date ? 'text-indigo-400' : 'text-slate-300'} />
                              {d}
                              {history[d].journal && <div className="w-1.5 h-1.5 rounded-full bg-pink-400 shadow-[0_0_5px_rgba(244,114,182,0.5)]" title="Has Journal Entry"></div>}
                        </div>
                        <button 
                            onClick={(e) => { 
                                e.stopPropagation(); 
